            ...
            <div class="max-w-7xl mx-auto">
                ...
                @forelse($orders as $order)
                    <div class="border rounded-lg p-4 bg-white shadow mb-6">
                        ...
                        {{-- 合計金額とステータス表示 --}}
                        <div class="mb-4">
                            <p class="text-gray-700">合計金額: {{ number_format($order->total_amount) }}円</p>
                            <div class="flex items-center space-x-2 mt-2">
                                @if($order->status === 'pending')
                                    <img src="{{ asset('images/status-pending.svg') }}" alt="保留中" class="w-8 h-8">
                                    <span class="text-sm text-gray-600">保留中</span>
                                @elseif($order->status === 'processing')
                                    <img src="{{ asset('images/status-processing.svg') }}" alt="購入手続き中" class="w-8 h-8">
                                    <span class="text-sm text-gray-600">購入手続き中</span>
                                @elseif($order->status === 'completed')
                                    <img src="{{ asset('images/status-completed.svg') }}" alt="発送準備中" class="w-8 h-8">
                                    <span class="text-sm text-gray-600">発送準備中</span>
                                @elseif($order->status === 'cancelled')
                                    <img src="{{ asset('images/status-cancelled.svg') }}" alt="キャンセル" class="w-8 h-8">
                                    <span class="text-sm text-gray-600">キャンセル</span>
                                @endif
                            </div>
                        </div>
